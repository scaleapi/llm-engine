...
from spellbook_serve.common.serialization_utils import bool_to_str
...
from spellbook_serve.domain.gateways import MonitoringMetricsGateway
from spellbook_serve.domain.repositories import DockerRepository
from spellbook_serve.domain.services import EndpointBuilderService
from spellbook_serve.domain.use_cases.model_endpoint_use_cases import (
    CONVERTED_FROM_ARTIFACT_LIKE_KEY,
)
from spellbook_serve.infra.gateways import FilesystemGateway
from spellbook_serve.infra.gateways.resources.endpoint_resource_gateway import (
    EndpointResourceGateway,
)
from spellbook_serve.infra.infra_utils import make_exception_log
from spellbook_serve.infra.repositories import FeatureFlagRepository, ModelEndpointCacheRepository
...
if LOCAL:
    with environment(KUBERNETES_SERVICE_HOST=None):
        logger = make_logger("spellbook_serve.service_builder")
else:
    logger = make_logger("spellbook_serve.service_builder")
...
class LiveEndpointBuilderService(EndpointBuilderService):
    ...
    def convert_artifact_like_bundle_to_runnable_image(
        ...
        new_flavor = RunnableImageFlavor(
            ...
            command=[
                ...
                "python",
                "-m",
                "spellbook_serve.inference.sync_inference.start_fastapi_server",
            ],
            env={
                "OMP_NUM_THREADS": '"1"',
                "BASE_PATH": "/app",
                ...
                "PORT": "5005",
                "ML_INFRA_SERVICES_CONFIG_PATH": f"/app/ml_infra_core/spellbook_serve.core/spellbook_serve.core/configs/{ml_infra_service_config_file}",
            },
            protocol="http",
        )
    ...
    def _get_base_image_params(
        ...
        inference_folder = "spellbook_serve/spellbook_serve/inference"
