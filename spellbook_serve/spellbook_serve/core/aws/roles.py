...
class AwsCredentials(NamedTuple):
    ...
    def client(self, client_type: str, region_name: str = "us-west-2") -> BaseClient:
        ...
        NOTE: Use the us-west-2 region unless you are absolutely sure you require a different region.
              All Scale AWS services are in US West 2.
        """
        return boto3.client(
            client_type,
...
def session(role: Optional[str], session_type: SessionT = Session) -> SessionT:
    ...
    Includes fall-back logic to work with setups that do not use a credentials file
    in the .aws folder in the user's home folder. In this setting, it is ok for :param:`role`
    to be an ARN. Otherwise, the `profile_to_arn` mapping in `scaleml.config` is used to
    locate the correct ARN for the given AWS profile name.

    NOTE: The fall-back is required for this to work with setups that use `aws-okta`.

    :raises: botocore.exceptions.ProfileNotFound, ValueError
    """
...
# returns scale user (e.g. pranav.pillai)
