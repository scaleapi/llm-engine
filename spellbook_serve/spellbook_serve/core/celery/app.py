...
from spellbook_serve.core.loggers import (
    CustomJSONFormatter,
    logger_name,
    make_logger,
...
backends.BACKEND_ALIASES["s3"] = "spellbook_serve.core.celery.s3:S3Backend"
...
class TaskVisibility(IntEnum):
    ...
    2. When making requests to such deployment, you'll have to do:
        ```python
        from scaleml.io.celery import TaskVisibility, celery_app
...
def get_redis_host_port():
    ...
    # In the case of k8s, pick the right endpoint based on the config
        ...
        host = ml_infra_config().redis_host
        port = 6379
    ...
    # In the case of local testing, pick the right endpoint based on the config
        ...
        logger.info(f"Inferring redis host from config env: {ml_infra_config().env}")
        host = f"redis-elasticache-message-broker.{ml_infra_config().dns_host_domain}"
        port = 6379
...
def celery_app(
    ...
    * mytask.apply_async(args=[], kwargs={}, soft_time_limit=30)
    ...
    "s3://<s3_bucket>/<s3_base_path>/...". Defaults to "scale-ml" (s3://scale-ml/tmp/celery/).

    :param s3_base_path: [optional] Base path for task results when using S3 as backend. The results uri will be
    "s3://<s3_bucket>/<s3_base_path>/...". Defaults to "tmp/celery/" (s3://scale-ml/tmp/celery/).
...
def _get_backend_url_and_conf(
    ...
    if backend_protocol == "redis":
        ...
        backend_url = "s3://"
        if aws_role is None:
            aws_session = session(ml_infra_config().profile_ml_worker)
        else:
