...
from spellbook_serve.core.aws import storage_client
...
S3_BUCKET = os.environ.get("S3_BUCKET", ml_infra_config().s3_bucket)
...
def zip_context(
    ...
    :param context: Path to context for dockerfile, relative to calling script, i.e. box_detection/ or scaleml/ if you're running from models/
...
@click.option(
    "--folders",
    required=False,
    help="Comma separated list of folders (relative to context), e.g. 'scaleml/,template-project/",
)
@click.option(
    "--no-cache",
    required=False,
