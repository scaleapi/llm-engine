...
def get_node_port(service_name: str) -> int:
    ...
    return int(hashlib.md5(service_name.encode()).hexdigest(), 16) % (32768 - 30000) + 30000
...
def get_target_concurrency_from_per_worker_value(per_worker: int) -> float:
    ...
    return per_worker / MAX_CONCURRENCY_TO_TARGET_CONCURRENCY_RATIO
...
def get_per_worker_value_from_target_concurrency(concurrency: Union[str, int, float]) -> int:
    ...
    return int(
        round(
            parse_cpu_request(str(concurrency))
            * MAX_CONCURRENCY_TO_TARGET_CONCURRENCY_RATIO
            / 1000.0
        )
    )
