...
async def test_create_async_endpoint_has_correct_labels(
    ...
    create_resources_request_async_custom: CreateOrUpdateResourcesRequest,
    create_resources_request_async_tensorflow: CreateOrUpdateResourcesRequest,
):
    for request in [
        create_resources_request_async_custom,
        create_resources_request_async_tensorflow,
    ]:
...
async def test_create_sync_endpoint_has_correct_labels(
    ...
    create_resources_request_sync_custom: CreateOrUpdateResourcesRequest,
):
    for request in [
    ...
    # Make sure that an VPA is created if optimize_costs is True.
    optimize_costs = create_resources_request_sync_pytorch.build_endpoint_request.optimize_costs
    create_vpa_call_args = mock_custom_objects_client.create_namespaced_custom_objects.call_args
    if optimize_costs:
        assert create_vpa_call_args is not None
    else:
