...
def _get_onprem_client_kwargs() -> Dict[str, Any]:
    ...
    # Try to get endpoint from config, fall back to env var
    try:
        from model_engine_server.core.config import infra_config

        s3_endpoint = getattr(infra_config(), "s3_endpoint_url", None)
    except Exception:
        s3_endpoint = None

    s3_endpoint = s3_endpoint or os.getenv("S3_ENDPOINT_URL")
    if s3_endpoint:
        client_kwargs["endpoint_url"] = s3_endpoint

    # Try to get addressing style from config, default to "path"
    try:
        from model_engine_server.core.config import infra_config
