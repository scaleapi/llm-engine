...
from model_engine_server.tracing.schema import (
    TraceConfig,
)
from model_engine_server.tracing.trace_ctx_var import (
    ctx_var_sgp_trace_queue_manager,
    ctx_var_sgp_trace_config,
)
...
SGP_TRACE_CONFIG_KWARG = "_sgp_trace_config"
TRACER_SGP_CLIENT_BASE_URL = os.environ.get(
    "TRACER_SGP_CLIENT_BASE_URL",
    "http://egp-api-backend.egp.svc.cluster.local:80/private/",
)
...
def get_encoded_trace_config(
    extra_metadata: Optional[dict[str, Any]] = None,
) -> Optional[str]:
    """
    Returns the encoded trace config from the current context.
    If no trace config is set, returns None.
    """
    queue_manager = ctx_var_sgp_trace_queue_manager.get()
    if queue_manager is None:
        return None
    caller_trace_config = ctx_var_sgp_trace_config.get()
    if caller_trace_config is None:
        # Note that this shouldn't happen, since if queue manager is set, the trace config should also be set.
        logger.warning("No trace config found in context, returning None.")
        return None
    ...
    return encode_trace_config(trace_config)


def get_encoded_trace_config_as_headers(
    encoded_trace_config: Optional[str] = None,
) -> dict[str, str]:
    return (
        {}
        if encode_trace_config is None
        else {SGP_TRACE_CONFIG_HEADER: encoded_trace_config}
    )
...
def get_trace_config_headers(
    ...
    encoded_trace_config = get_encoded_trace_config(extra_metadata)
    return get_encoded_trace_config_as_headers(encoded_trace_config)
...
def encode_trace_config(trace_config: TraceConfig) -> str:
    ...
    return base64.b64encode(
        json.dumps(
            trace_config.model_dump(), sort_keys=True, separators=(",", ":")
        ).encode("utf-8")
    ).decode("utf-8")
...
def decode_trace_config(serialized_trace_config: str) -> TraceConfig:
    ...
    """
    decoded_str = base64.b64decode(serialized_trace_config.encode("utf-8")).decode(
        "utf-8"
    )
    return TraceConfig.model_validate(json.loads(decoded_str))


def safe_decode_trace_config(
    serialized_trace_config: Optional[str],
    ...
    if serialized_trace_config is None:
        return None
    try:
        return decode_trace_config(serialized_trace_config)
    except Exception as e:
        logger.error(f"Failed to decode trace config: {e} - {serialized_trace_config}")
    return None


def init_trace_queue_manager(
    trace_config: TraceConfig,
) -> trace_queue_manager.TraceQueueManager:
    ...
    queue_manager = trace_queue_manager.TraceQueueManager(
        client=tracer_client, worker_enabled=False
    )
    trace = Trace(
...
def init_tracing(
    serialized_trace_config: Optional[str],
) -> Optional[trace_queue_manager.TraceQueueManager]:
    """
    Adds a trace context to the stack based on the SGP trace config header value.
    """
    try:
        trace_config = safe_decode_trace_config(serialized_trace_config)
