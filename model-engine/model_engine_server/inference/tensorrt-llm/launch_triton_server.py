...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        default=5005,
        help=
        'Default HTTP port to 5005. See llm-engine/model-engine/model_engine_server/inference/configs/service--http_forwarder.yaml'
    )
...
def get_cmd(world_size, tritonserver, model_repo, http_port):
    ...
    for i in range(world_size):
        ...
        cmd += f' -n 1 {tritonserver} --model-repository={model_repo} --http-port {http_port} --disable-auto-complete-config --backend-config=python,shm-region-prefix-name=prefix{i}_ : '
    cmd += '&'
    return cmd
