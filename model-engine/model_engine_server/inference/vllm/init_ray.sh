...
ray_port=6379
ray_init_timeout=1200  # Needs to be large enough to overcome any skew from the s5cmd command + any pod startup time
...
case "$subcommand" in
  worker)
    ...
    for (( i=0; i < $ray_init_timeout; i+=5 )); do
      ray start --address=$ray_address:$ray_port --block --node-ip-address=$own_address
      if [ $? -eq 0 ]; then
  ...
  leader)
    ...
    ray start --head --port=$ray_port --node-ip-address=$own_address
