...
def run_download(download_cmd: str, model_dir: str) -> dict:
    ...
    # download_cmd is generated by model-engine from trusted endpoint config. Run it under bash so
    # the generated fail-fast flags, including pipefail, behave the same as normal endpoint startup.
    subprocess.run(["/bin/bash", "-c", download_cmd], check=True)
...
def get_model_dir_from_server_args(server_args: list[str]) -> str:
    try:
        return server_args[server_args.index("--model") + 1]
...
def main():
    ...
    model_dir = get_model_dir_from_server_args(server_args)

    if download_cmd:
        timing = run_download(download_cmd, model_dir)
