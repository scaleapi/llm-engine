...
def create_parent_context(pod_uid: str, parent_suffix: str = "root") -> Optional[Context]:
    """Create a trace context with a specific parent span.

    This is the core utility for trace correlation. It creates a context where
    the parent span has a deterministic span ID derived from pod_uid:parent_suffix.

    Usage:
        # Create context where "root" is the parent
        ctx = create_parent_context(pod_uid, "root")

        # Create context where "in_container_startup" is the parent
        ctx = create_parent_context(pod_uid, "in_container_startup")

    Args:
        pod_uid: Kubernetes pod UID
        parent_suffix: Span suffix for deriving parent span ID (default "root")
    ...
    span_id = derive_span_id(pod_uid, parent_suffix)
...
# Convenience aliases for common parent contexts
def create_deterministic_context(pod_uid: str) -> Optional[Context]:
    """Create context with deterministic root as parent. Alias for create_parent_context(pod_uid, 'root')."""
    return create_parent_context(pod_uid, "root")
...
def create_k8s_parent_context(pod_uid: str) -> Optional[Context]:
    """Create context with k8s_total_to_running as parent. Alias for create_parent_context(pod_uid, SPAN_K8S_TOTAL_TO_RUNNING)."""
    return create_parent_context(pod_uid, SPAN_K8S_TOTAL_TO_RUNNING)


def create_in_container_parent_context(pod_uid: str) -> Optional[Context]:
    """Create context with in_container_startup as parent. Alias for create_parent_context(pod_uid, SPAN_IN_CONTAINER_STARTUP)."""
    return create_parent_context(pod_uid, SPAN_IN_CONTAINER_STARTUP)
