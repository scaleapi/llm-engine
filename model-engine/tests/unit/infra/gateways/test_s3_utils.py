...
def mock_infra_config_aws():
    with mock.patch("model_engine_server.core.config.infra_config") as mock_config:
...
def test_get_s3_client_aws(mock_session, mock_infra_config_aws):
    ...
    # Ensure S3_ENDPOINT_URL is not set for this test
...
@mock.patch("model_engine_server.infra.gateways.s3_utils.boto3.Session")
def test_get_s3_client_aws_with_endpoint_url(mock_session, mock_infra_config_aws):
    """Test that S3_ENDPOINT_URL works even in AWS mode (for CircleCI/MinIO compatibility)."""
    mock_client = mock.Mock()
    mock_session.return_value.client.return_value = mock_client

    with mock.patch.dict(os.environ, {"S3_ENDPOINT_URL": "http://minio:9000"}):
        result = get_s3_client()
...
@mock.patch("model_engine_server.infra.gateways.s3_utils.boto3.Session")
def test_get_s3_resource_aws_with_endpoint_url(mock_session, mock_infra_config_aws):
    """Test that S3_ENDPOINT_URL works even in AWS mode for resource (for CircleCI/MinIO compatibility)."""
    mock_resource = mock.Mock()
    mock_session.return_value.resource.return_value = mock_resource
...
@mock.patch("model_engine_server.infra.gateways.s3_utils.boto3.Session")
def test_get_s3_client_config_failure_fallback(mock_session):
    """Test that S3 client falls back to AWS behavior when config fails."""
    with mock.patch("model_engine_server.core.config.infra_config") as mock_config:
        ...
        assert result == mock_client
        # Should fall back to AWS behavior
        mock_session.assert_called_with(profile_name="test-profile")
        mock_session.return_value.client.assert_called_with("s3")
