...
def test_load_model_weights_sub_commands(
    ...
    if "llama-2-13b-safetensors-path" in checkpoint_path:
        assert (
            subcommands[-1]
            == f"./s5cmd --numworkers 512 cp --concurrency 10 --exclude '*.bin' {os.path.join(checkpoint_path, '*')} ''"
        )
    elif "llama-2-13b-bin-path" in checkpoint_path:
        assert (
            subcommands[-1]
            == f"./s5cmd --numworkers 512 cp --concurrency 10 --exclude '*.safetensors' {os.path.join(checkpoint_path, '*')} ''"
        )
