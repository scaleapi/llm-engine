...
def launch_celery_app(
    ...
    env = dict(
        AWS_PROFILE="default" if os.getenv("CIRCLECI") else infra_config().profile_ml_worker,
        BROKER_TYPE="redis",
        USE_REDIS_LOCALHOST=1,
        CELERY_S3_BUCKET=infra_config().s3_bucket,
        RESULTS_S3_BUCKET=infra_config().s3_bucket,
        CHILD_FN_INFO="{}",
        BASE_PATH=str(BASE_PATH),
        PREWARM=True,
        BUNDLE_URL=f"s3://{infra_config().s3_bucket}/model_bundles/61a67d767bce560024c7eb96/f0142411-51e1-4357-a405-ee5fef87d977",
    ...
    command = (
        f"{env_str} exec celery --app=model_engine_server.inference.async_inference worker "
