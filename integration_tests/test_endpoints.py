...
def test_async_model_endpoint(user, create_endpoint_request, update_endpoint_request, inference_requests):
    try:
...
@pytest.mark.parametrize("user", [USER_ID_0, USER_ID_1])
def test_sync_model_endpoint(user):
    try:
...
@pytest.mark.parametrize("user", [USER_ID_0, USER_ID_1])
def test_sync_streaming_model_endpoint(user):
    try:
