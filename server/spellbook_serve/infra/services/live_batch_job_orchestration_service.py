...
from spellbook_serve.domain.gateways import AsyncModelEndpointInferenceGateway
from spellbook_serve.domain.services import ModelEndpointService
from spellbook_serve.domain.use_cases.async_inference_use_cases import DEFAULT_TASK_TIMEOUT_SECONDS
...
from spellbook_serve.infra.services.batch_job_orchestration_service import (
...
class LiveBatchJobOrchestrationService(BatchJobOrchestrationService):
    ...
    async def _submit_tasks(
        ...
        with self.filesystem_gateway.open(
            input_path, "r", aws_profile=ml_infra_config().profile_ml_worker
        ) as f:
    ...
    def _serialize_and_write_results(
        ...
        with self.filesystem_gateway.open(
            result_location, "wb", aws_profile=ml_infra_config().profile_ml_worker
        ) as f:
