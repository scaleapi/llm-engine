...
from llm_engine_server.inference.service_requests import make_request
...
def is_sensitive_envvar(var):
    return var.startswith("LLM_ENGINE_") or var.startswith("HMI_")


def unset_sensitive_envvars():
    # Since all the pods are in the same namespace as of now, there are env vars e.g.
    # `LLM_ENGINE_<USER_ID>_...` that store the IPs of various services (and also leak that the services exist)
    # Let's unset them here
    # The names seem to be the name of the deployment, which always starts with `LLM_ENGINE_` or `HMI_`.
