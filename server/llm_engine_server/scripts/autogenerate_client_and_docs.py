...
LLM_ENGINE_SERVICE_BASE = MODULE_PATH.parents[2].resolve()
OPENAPI_PATH = (LLM_ENGINE_SERVICE_BASE / "clients/openapi.json").resolve()
...
def run_openapi_generator():
    ...
    subprocess.run(
        command,
        cwd=str((LLM_ENGINE_SERVICE_BASE / "../ml_infra_core").resolve()),
        check=True,
        shell=True,
    )
